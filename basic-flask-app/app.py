...
@app.route("/behav")
def behav_desc():
    return render_template("behav.html")
...
def upload():
    ...
    try:
        ...
        desc["Intezer API"] = "Detect the ransomware by Intezer Vendor"
        desc["Scanii API"] = "Detect the ransomware by Scanii Vendor"
        #ss = solution_deja_existante(filename)
        try :
            if len(ss) == 2:
                for i in range(len(ss[0])):
                    data[ss[0][i]] = ss[1][i]
                    desc[ss[0][i]] = "Detect the ransomware by "+ str(ss[0][i]) +" Vendor"
        except TypeError:
            data["Smart analysis"] = ss
            desc["Smart analysis"] = "Detect the ransomware by Smart analysis Vendor"
        ...
        # Generate the report
        path_to_pdf, rate = generate_report(data,s,familly,filename)
        # DELETE THE EXTRACT INFO + UPLOADS FILES
        delete_all(filename)
        send_file(path_to_pdf, as_attachment=True)
        print("Total rate is : "+str(rate))
        return render_template("results.html", data=data, desc=desc, details=details, path_to_pdf=path_to_pdf, rate=rate)
