...
def load_and_read_json2():
    ...
    # get the exe file
    for i in get_files :
        if "behaviour" in i and ".json" in i:
            return i
    print("ERROR !!!")
...
def draw_graph(hash_file):
    try :
        API_KEY = "bd8c0289502c65de06bb2b737a002197414b376ccfb21ea936d24eb7a0d71d68"  # Insert your VT API here.
        # Creates the graph.
...
def generate_report(data,s,familly,filename):
    ...
    os.chdir('/var/www/basic-flask-app/archive')
    pdf_file.save()
    print("["+str(now)+"]~ Done!!! - THE PDF HAS BEEN GENERATED")
    return get_pdf(), score / total